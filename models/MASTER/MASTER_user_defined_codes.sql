...
select
  ...
  case
    when length(DRUPMT) = 1
      then to_time(time_from_parts(0, 0, DRUPMT))
    when length(DRUPMT) = 2
      then to_time(time_from_parts(0, 0, DRUPMT))
    when length(DRUPMT) = 3
      then to_time(time_from_parts(0, substring(DRUPMT, 1, 1), substring(DRUPMT, 2, 2)))
    when length(DRUPMT) = 4
      then to_time(time_from_parts(0, substring(DRUPMT, 1, 2), substring(DRUPMT, 3, 2)))
    when length(DRUPMT) = 5
      then to_time(time_from_parts(truncate((DRUPMT / 10000), 0), substring(DRUPMT, 2, 2), substring(DRUPMT, 4, 2)))
    when length(DRUPMT) = 6
      then to_time(time_from_parts(truncate((DRUPMT / 10000), 0), substring(DRUPMT, 3, 2), substring(DRUPMT, 5, 2)))
